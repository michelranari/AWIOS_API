...
router.delete('/', (req,res) =>{
  ...
  jwt.verify(token, process.env.JWT_KEY , function(err, decoded) {
    ...
    // call function to delete tag
    return del(req.body.id,req.body.toDelete,res);
...
function del(identifiant,del,res){
  ...
  tagModel.findById(identifiant, function(err1,tag){
    ...
    if(tag.nbOccurence > 1){
      ...
      if(prop){
        ...
        tagModel.findOneAndUpdate({"_id" : tag._id},update,{new: true}, function(err2,tag1){
          ...
          propositionModel.findById({_id : toDelete},function(err3,prop1){
            ...
            var contentProp = prop1.tagsProp.filter(id => id != identifiant);
      ...
      // if is in a answer
        ...
        answerModel.findById(toDelete,function(err6,prop3){
          ...
          var contentProp = prop3.tagsAnswer.filter(id => id != identifiant);
    ...
    // if appear 1 time : delete the tag
      ...
      tagModel.findByIdAndRemove(identifiant,function(err8,deleted){
        ...
        // delete the tag in proposition array tagsProp
        if(prop){
          propositionModel.findById({_id : toDelete},function(err9,prop5){
            ...
            var contentProp = prop5.tagsProp.filter(id => id != identifiant);
            var update = {"tagsProp" : contentProp }
          ...
          answerModel.findById({_id : toDelete},function(err11,prop7){
            ...
            var contentProp = prop7.tagsAnswer.filter(id => id != identifiant);
            var update = {"tagsAnswer" : contentProp }
